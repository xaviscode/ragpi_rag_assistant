...
def retrieve(settings: Settings, store: VectorStore, embedder: Embedder, question: str, document_ids: list[str] | None = None, query_text: str | None = None) -> dict[str, Any]:
    ...
    res = store.query(query_embedding=qemb, n_results=n_results, where=where, include_embeddings=settings.mmr_enabled)

    docs = (res.get("documents") or [[]])[0]
    metas = (res.get("metadatas") or [[]])[0]
    ids = (res.get("ids") or [[]])[0]
    dists = (res.get("distances") or [[]])[0]
    embs = (res.get("embeddings") or [[]])[0] if settings.mmr_enabled else [None] * len(docs)

    candidates = []
    for doc, meta, cid, dist, emb in zip(docs, metas, ids, dists, embs):
        meta = meta or {}
        if settings.distance_threshold is not None and dist is not None and float(dist) > settings.distance_threshold:
            continue
        candidates.append({
            "chunk_id": cid,
            "text": doc,
    ...
    items = _mmr_select(candidates, qemb, settings.top_k, settings.mmr_lambda) if settings.mmr_enabled else candidates[:settings.top_k]
    for item in items:
        item.pop("embedding", None)
    return {"items": items, "query_text": effective_query, "raw_candidates": len(docs), "filtered_candidates": len(candidates)}
